import React, { useContext, useEffect, useState } from "react";
import { SidebarContext } from "../contexts/SidebarContext";
import { CartContext } from "../contexts/CartContext";
import { Link } from "react-router-dom";
import Logo from "../img/logo-greenleaf.png";
import { BsBag } from "react-icons/bs";
import { BsList } from "react-icons/bs"; // Hamburger icon

const Header = () => {
  // header state
  const [isActive, setIsActive] = useState(false);
  const [isMenuOpen, setIsMenuOpen] = useState(false); // To toggle the mobile menu
  const { isOpen, setIsOpen } = useContext(SidebarContext);
  const { itemAmount } = useContext(CartContext);

  // event listener
  useEffect(() => {
    window.addEventListener("scroll", () => {
      window.scrollY > 60 ? setIsActive(true) : setIsActive(false);
    });
  }, []);

  return (
    <header
      className={`${isActive ? "bg-white py-4 shadow-md" : "bg-none py-6"
        } fixed w-full z-10 transition-all duration-300 lg:px-8`}
    >
      <div className="container mx-auto flex items-center justify-between h-full header-img">
        {/* Logo */}
        <Link to={"/"}>
          <div className="w-[200px]">
            <img src={Logo} alt="Green Life Logo" />
          </div>
        </Link>

        {/* Navigation Links for Desktop */}
        <div className="hidden md:flex gap-8">
          <Link to={"About"} className="text-sm md:text-base font-semibold hover:text-primary">
            Biz Haqimizda
          </Link>
          <Link to={"Sozlamalar"} className="text-sm md:text-base font-semibold hover:text-primary">
            Sozlamalar
          </Link>
          <Link to={"Operator"} className="text-sm md:text-base font-semibold hover:text-primary">
            Operator
          </Link>
        </div>

        {/* Cart Icon */}
        <div
          onClick={() => setIsOpen(!isOpen)}
          className="cursor-pointer flex relative"
        >
          <BsBag className="text-2xl" />
          <div className="bg-red-500 absolute -right-2 -bottom-2 text-[12px] w-[18px] h-[18px] text-white rounded-full flex justify-center items-center">
            {itemAmount}
          </div>
        </div>

        {/* Hamburger Menu (Mobile) */}
        <div className="md:hidden flex items-center">
          <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="text-2xl">
            <BsList />
          </button>
        </div>
      </div>

      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden flex flex-col items-center py-4 shadow-md absolute top-[70px] left-0 right-0 z-20 link">
          <Link to={"About"} className="text-white text-sm font-semibold hover:text-gray-200 py-2">
            Biz Haqimizda
          </Link>
          <Link to={"Sozlamalar"} className="text-white text-sm font-semibold hover:text-gray-200 py-2">
            Sozlamalar
          </Link>
          <Link to={"Operator"} className="text-white text-sm font-semibold hover:text-gray-200 py-2">
            Operator
          </Link>
        </div>
      )}
    </header>
  );
};

export default Header;
